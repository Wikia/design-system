...
module('Integration | Component | wds-on-site-notifications/notifications-list with loaded results', function (hooks) {
	...
	test('it should render a proper number of notification-card components', async function (assert) {
		...
		assert.equal(this.element.querySelectorAll('.wds-notification-card').length, 2,
			'should render two wds-notification-card components');
	});
...
module('Integration | Component | wds-on-site-notifications/notifications-list when loading results', function (hooks) {
	setupRenderingTest(hooks);
	...
	test('it renders as a UL element with proper classnames', async function (assert) {
		await render(hbs`{{wds-on-site-notifications/notifications-list}}`);
